...
class ChatbotApp:
    def __init__(self, root):
        ...
        self.Rupert_frame = tk.Frame(root, bg='#f39109', bd=2, relief="groove")
        self.Rupert_frame.place(x=10, y=10, width=200, height=230)

        self.Rupert_label = tk.Label(self.Rupert_frame, text="Rupert", bg='#f39109', fg="black", font=("Arial",12, "bold"))
        self.Rupert_label.pack()

        self.load_Rupert_image()
        ...
        self.text_display = tk.Text(root, bg='#f79a24', fg='black', font=("Arial", 12), wrap='word')
        self.text_display.place(x=220, y=10, width=570, height=500)
        self.text_display.config(state=tk.DISABLED)

        self.input_field = tk.Entry(root, bg='#FFFFFF', fg='black', font=("Arial", 12))
        self.input_field.place(x=10, y=520, width=780, height=50)
        self.input_field.bind("<Return>", self.process_command)

        self.append_text("Rupert: Hello, I'm Rupert. How can I assist you today?")
        self.speak("Hello, I'm Rupert. How can I assist you today?")

    def load_Rupert_image(self):
        self.Rupert_img = tk.PhotoImage(file="images/rupert.png")  
        self.Rupert_image_label = tk.Label(self.Rupert_frame, image=self.Rupert_img, bg='#d9b0d9')
        self.Rupert_image_label.pack()

    def toggle_voice_mode(self):
        self.voice_mode = not self.voice_mode

        if self.voice_mode:
            self.append_text("Rupert: Voice mode activated.")
            self.speak("Voice mode activated.")
            threading.Thread(target=self.listen_voice).start()
        else:
            self.append_text("Rupert: Voice mode deactivated.")
    ...
    def listen_voice(self):
        ...
        with sr.Microphone() as source:
            self.append_text("Rupert is listening...")
        ...
        try:
            ...
            self.append_text("Rupert: Sorry, I didn't catch that.")
            self.speak("Sorry, I didn't catch that.")
        except sr.RequestError:
            self.append_text("Rupert: Sorry, there seems to be an issue with the voice service.")
    ...
    def process_command_common(self, command):
        if "time" in command:
            ...
            self.append_text(f"Rupert: {response}")
            self.speak(response)
